...
##############################################################################################################################
# 随机生成数据,用来实验的, 进而进行random sampling 实验 有noise
np.random.seed(2)
proportion=0.9
sample_size=200
# 10%的minority为噪声  新增比较好
size_C0=int(sample_size*proportion)
size_C1=int(sample_size*(1-proportion))
size_noise=int(size_C1*0.2)
print(size_noise)
C0 = np.random.normal(loc=0, scale=4, size=size_C0)
y0 = [-1]*size_C0
C1 = np.random.normal(loc=2, scale=1, size=size_C1)
C_noise=np.random.normal(loc=0,scale=1,size=size_noise)
# using the addition to generate the noise data
a=np.array(C1[0:size_noise])
b=np.array(C_noise)
C_noise=a+b
C1=np.append(C1,C_noise)
size_C1=size_C1+size_noise
y1 = [1] *size_C1
C=np.append(C0,C1)
y=np.append(y0,y1)
...
for name, clf in classifiers.items():
    for i in range(100):
        ...
        oversamp = RandomUnderSampler(random_state=23)
